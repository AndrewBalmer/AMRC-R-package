...
if (!dir.exists(data_dir)) {
  stop(
    "Local data directory not found at ",
    data_dir,
    ". Recover the extracted legacy files before knitting this notebook.",
    call. = FALSE
  )
}
...
load_local_rdata <- function(filename, env = knitr::knit_global()) {
  load(file.path(data_dir, filename), envir = env)
}

# Read the MIC table data
tablemic <- read_local_csv("MIC_table_Spneumoniae.csv", header=TRUE, sep=",", skip = 0)

# Read in the relevant meta data
tablemic_meta <- read_local_csv("meta_data_Spneumoniae.csv", header=TRUE, sep=",", skip = 0)

# Load the precomputed PCA start 2D metric data
load_local_rdata("Spneumo_3628_PCA_start_2D_METRIC.RData")
...
for (i in 1:ncol(tablemic)) {
  ...
  # Load the pre-computed PCA start 2D metric data
  load_local_rdata("Spneumo_3628_PCA_start_2D_METRIC_genetic.RData")
...
tablemic_metadata_MLST <- read_local_csv("MIC_S.Pneumo_metadata.csv", header=TRUE, sep=",", skip = 0)
tablemic_metadata_MLST_post2015 <- read_local_csv("Meta_data_spneumoniae_isolates_post_2015.csv", header=TRUE, sep=",", skip = 0) %>%
  rename(LABID = Sample,
         MLST = ST,
