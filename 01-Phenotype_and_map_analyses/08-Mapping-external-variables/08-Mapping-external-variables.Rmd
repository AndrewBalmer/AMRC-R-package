...
output_dir <- file.path(repo_root, "01-Phenotype_and_map_analyses", "08-Mapping-external-variables")
example_dir <- system.file("extdata", "examples", "spneumoniae-08", package = "amrcartography")
if (!nzchar(example_dir)) {
  example_dir <- file.path(repo_root, "inst", "extdata", "examples", "spneumoniae-08")
}

if (!dir.exists(example_dir)) {
  stop(
    "Packaged 08 case-study data directory not found at ",
    example_dir,
    ". Build it with tools/build_spneumoniae_08_case_study_bundle.R before knitting this notebook.",
    call. = FALSE
  )
}

read_example_csv <- function(filename, ...) {
  read.csv(file.path(example_dir, filename), ...)
}

maps_bundle <- readRDS(file.path(example_dir, "spneumoniae_08_map_bundle.rds"))

read_case_study_table <- function(filename, ...) {
  read_example_csv(filename, stringsAsFactors = FALSE, check.names = FALSE, ...)
}

# Read the MIC table data
tablemic <- read_case_study_table("MIC_table_Spneumoniae.csv", header=TRUE, sep=",", skip = 0)

# Read in the relevant meta data
tablemic_meta <- read_case_study_table("meta_data_Spneumoniae.csv", header=TRUE, sep=",", skip = 0)
...
slope <- maps_bundle$phenotype_slope
dilation <- maps_bundle$phenotype_dilation
slope

# The packaged case-study bundle stores the already dilated and rotated
# phenotype map used throughout this notebook.
phen_map <- as_tibble(maps_bundle$phenotype_map) %>%
  dplyr::select(D1, D2)
...
ggsave(file.path(output_dir, "MIC_spneumo_phen_distribution.jpg"), plot = combined_plot, width = 12, height = 10, dpi = 600)
...
ggsave(file.path(output_dir, "Mean_MIC_spneumo_phen_distribution.jpg"))
...
ggsave(file.path(output_dir, "Penicillin_S_pneumo_biplot.jpg"), width = 10, height = 10, dpi = 600)
...
ggsave(file.path(output_dir, "Combined_Biplot_Mean_MIC_spneumo_phen_distribution.jpg"), 
...
isolates_with_PBP_deletion <- maps_bundle$deleted_labids
map_coords <- as.data.frame(dplyr::select(maps_bundle$phenotype_map, D1, D2))
genotype_mds <- as.data.frame(dplyr::select(maps_bundle$genotype_map, G1, G2))

### create main dataframe of map coordinates and PBP metadata
AA_matrix_phenotype <- as_tibble(maps_bundle$phenotype_map) %>%
                         rename(PBP_type = PT) %>%
                         group_by(PBP_type) %>%
...
gen_slope <- maps_bundle$genotype_slope
...
gen_dilation <- maps_bundle$genotype_dilation
gen_slope
...
res.hcpc <- HCPC(dplyr::select(gen_clustering, G1,G2), nb.clust = 12, consol = T, iter.max = 500, graph = T, order = T)
...
AA_matrix_phenotype <- left_join(AA_matrix_phenotype, dplyr::select(gen_clustering, PBP_type, gen_cluster), by = "PBP_type")
...
tablemic_metadata_MLST <- read_case_study_table("MIC_S.Pneumo_metadata.csv", header=TRUE, sep=",", skip = 0)
tablemic_metadata_MLST_post2015 <- read_case_study_table("Meta_data_spneumoniae_isolates_post_2015.csv", header=TRUE, sep=",", skip = 0) %>%
  rename(LABID = Sample,
         MLST = ST,
...
ggsave(file.path(output_dir, "Spneumo_MLSTphen_distribution.jpg"))
...
ellipse_groups <- split(tablemic_metadata_MLST, tablemic_metadata_MLST$MLST_with_count)
ellipse_groups <- ellipse_groups[vapply(ellipse_groups, function(df) {
  if (nrow(df) < 3L) {
    return(FALSE)
  }

  coords <- as.matrix(df[, c("D1", "D2"), drop = FALSE])
  ...
  is.matrix(cov_mat) &&
    all(is.finite(cov_mat)) &&
    abs(det(cov_mat)) > 1e-8
}, logical(1))]
ellipse_data <- dplyr::bind_rows(ellipse_groups)
...
p_facets <- ggplot(tablemic_metadata_MLST, aes(x = x_centroid, y = y_centroid)) +
  ...
  geom_mark_ellipse(data = ellipse_data, aes(x = D1, y = D2, fill = MLST_with_count),
...
# Save 
ggsave(file.path(output_dir, "Spneumo_MLSTphen_distribution_multipanel.jpg"),
       plot = p_facets, width = w, height = h, units = "in", dpi = 600, bg = "white")

# Optional vector export for PowerPoint (crisp, non-squished)
# ggsave(file.path(output_dir, "Spneumo_MLSTphen_distribution_multipanel.pdf"), plot = p_facets,
...
```{r,echo = F}
ellipse_groups_gen <- split(tablemic_metadata_MLST, tablemic_metadata_MLST$MLST_with_count)
ellipse_groups_gen <- ellipse_groups_gen[vapply(ellipse_groups_gen, function(df) {
  if (nrow(df) < 3L) {
    return(FALSE)
  }

  coords <- as.matrix(df[, c("G1", "G2"), drop = FALSE])
  coords <- coords[stats::complete.cases(coords), , drop = FALSE]
  if (nrow(coords) < 3L) {
    return(FALSE)
  }
...
ellipse_data_gen <- dplyr::bind_rows(ellipse_groups_gen)
...
ggplot(tablemic_metadata_MLST, aes(x = G1, y = G2)) +
  geom_point(data = AA_matrix_phenotype, aes(x=G1,y=G2), colour = "#999999", fill = "#999999", shape = 16, size = 1, alpha = .8) +
    geom_mark_ellipse(data = ellipse_data_gen, aes(x = G1, y = G2, fill = MLST_with_count), color = "black", expand = unit(2.5, "mm"),alpha = 0.2) +
  geom_point(aes(fill = MLST_with_count), shape = 21,size = 2.5, alpha =1, colour = "black") + 
  facet_wrap(MLST_with_count ~. ) + 
  scale_x_continuous(limits = limits_gen_map_x, breaks=breaks_gen_map_x) +
  scale_y_continuous(limits = limits_gen_map_y, breaks=breaks_gen_map_y) +
...
ggsave(file.path(output_dir, "Spneumo_MLSTgen_distribution.jpg"))
...
centroids_202 <- tablemic_metadata_MLST %>% 
  ...
  distinct(PBP_type, .keep_all = TRUE) %>% 
  ungroup() %>%
  dplyr::select(x_centroid, y_centroid, G1, G2)
...
ggsave(
  file.path(output_dir, "Spneumo_gen_by_phen_distance_MEDIANcentroids_top12_coloured_others_grey_WITH_MLST_UNWEIGHTED_PBP_MEANS.jpg"),
