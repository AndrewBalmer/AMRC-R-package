...
MLST = pandas.read_csv('S.pneumo_MLST_for_GWAS.csv')
MLST_dummy = pandas.get_dummies(MLST['MLST'].astype(str), drop_first=True)
...
### GWAS while accounting for MLST type
import pandas as pd
import numpy as np
...
# Load data
MLST = pd.read_csv('S.pneumo_MLST_for_GWAS.csv')
MLST_dummies = pd.get_dummies(MLST['MLST'].astype(str), drop_first=True)

# Fixed effects matrix (M): MLST only
M = MLST_dummies

# Covariance terms for traits (Y = map_coords)
A = np.eye(map_coords.shape[1])  # Trait covariance
A0 = np.ones((map_coords.shape[1], 1))  # Intercept
A1 = np.eye(map_coords.shape[1])  # Trait-specific variance

# Prepare results
pv_mv = pd.DataFrame(columns=["lml0", "lml2", "dof20", "scale2", "pv20"])
eff_mv_h2 = pd.DataFrame(columns=["test", "trait", "effect_type", "effect_name", "env", "effsize", "effsize_se"])

# GWAS loop with NA fallback for failed markers
for i in range(genetic_markers.shape[1]):
    test_marker = genetic_markers.iloc[:, i].to_frame()
    try:
        K = dummy_gen.drop(test_marker.columns[0], axis=1)
        K = linear_kinship(K.to_numpy())
        result = scan(G=test_marker, Y=map_coords.to_numpy(), K=K, M=M, A=A, A0=A0, A1=A1, verbose=False)
        pv_mv = pd.concat([pv_mv, result.stats], ignore_index=True)
        eff_mv_h2 = pd.concat([eff_mv_h2, result.effsizes['h2']], ignore_index=True)
    except np.linalg.LinAlgError as e:
        print(f"Skipping marker {i}: {str(e)}")
        # Fill with NA row for stats
        empty_stats = pd.DataFrame({col: [np.nan] for col in pv_mv.columns})
        pv_mv = pd.concat([pv_mv, empty_stats], ignore_index=True)
        # Fill with NA row for h2 effect sizes
        empty_eff = pd.DataFrame({col: [np.nan] for col in eff_mv_h2.columns})
        eff_mv_h2 = pd.concat([eff_mv_h2, empty_eff], ignore_index=True)

# Save results
pv_mv.to_csv('mvLMM_p_values_with_MLST.csv', index=False)
eff_mv_h2.to_csv('mvLMM_effect_sizes_with_MLST.csv', index=False)

# Report Bonferroni hits
bonferroni_threshold = 5.88e-4
n_hits = (pv_mv['pv20'].astype(float) < bonferroni_threshold).sum()
print(f"\nNumber of Bonferroni-significant hits: {n_hits}")
...
pv_mv_epi_full.pv20.min()


map_coords_full.to_csv('mvLMM_map_coords_normal_pneumo_random_phenotype_EPI_FWAS_91_100.csv')
pv_mv_epi_full.to_csv('mvLMM_p_values_normal_pneumo_random_phenotype_EPI_FWAS_91_100.csv')
eff_mv_h2_epi_full.to_csv('mvLMM_effect_sizes_normal_pneumo_random_phenotype_EPI_FWAS_91_100.csv')
...
### Additional inputs for genotype to phenotype prediction model (not used in publication)
import scipy as sp
import numpy as np
